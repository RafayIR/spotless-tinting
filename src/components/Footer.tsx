...
export default function Footer() {
  return (
    <footer className="relative z-20 isolate border-t border-ink-100 bg-ink-50 text-ink-600 transition-colors dark:border-ink-800 dark:bg-ink-950 dark:text-ink-300">
      <div className="container py-16">
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4">
          ...
          {/* Navigation */}
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-ink-900 dark:text-white">Navigation</h3>
            <ul className="mt-4 space-y-2.5 text-sm">
              <li><Link to="/" className="hover:text-accent-600 dark:hover:text-accent-400">Home</Link></li>
              <li><Link to="/about" className="hover:text-accent-600 dark:hover:text-accent-400">About</Link></li>
          ...
          {/* Services */}
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-ink-900 dark:text-white">Services</h3>
            <ul className="mt-4 space-y-2.5 text-sm">
              {services.map((s) => (
                <li key={s.id}>
                  <Link to={`/services/${s.slug}`} className="hover:text-accent-600 dark:hover:text-accent-400">{s.name}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Hours + Legal + Social */}
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-ink-900 dark:text-white">Business Hours</h3>
            <ul className="mt-4 space-y-1.5 text-sm">
              {business.hours.map((h) => (
                <li key={h.day} className="flex justify-between gap-2">
                  <span className="text-ink-500 dark:text-ink-400">{h.day}</span>
                  <span>{h.hours}</span>
                </li>
              ))}
            </ul>
            <h3 className="mt-6 text-sm font-semibold uppercase tracking-wider text-ink-900 dark:text-white">Legal</h3>
            <ul className="mt-4 space-y-2.5 text-sm">
            ...
            <div className="mt-6 flex gap-3">
              ...
              <a href={business.social.instagram} aria-label="Instagram" className="rounded-full bg-white p-2.5 text-ink-500 ring-1 ring-ink-200 hover:bg-accent-500 hover:text-white dark:bg-ink-900 dark:text-ink-300 dark:ring-ink-700">
